...
data = pd.read_csv('imputation.csv')
data = data.drop(['sample_name', 'storage_time'], axis=1)
data.head()
...
imp_df = rfImputer(data)
imp_df.impute('random_forest', {'n_estimators': 1000, 'n_jobs': 1,})

out = imp_df.imputed_df()

out.head()
out.to_csv("imputation_processed.csv")
